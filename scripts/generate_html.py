  ...
  <div id="app">
    <div id="viewport">
      <div id="world">
        ...
        <div class="axis-meta-label" id="label-arousal-top">↑ Arousal</div>
        <div class="axis-meta-label" id="label-arousal-bottom">↓ Arousal</div>
        <div class="axis-meta-label" id="label-valenz-left">← Valenz</div>
        <div class="axis-meta-label" id="label-valenz-right">Valenz →</div>
...
const labelGap2 = labelGap + 92;
widget.querySelector('#label-arousal-top').style.left = midX + 'px';
widget.querySelector('#label-arousal-top').style.top = (contentMinY - labelGap2) + 'px';
widget.querySelector('#label-arousal-bottom').style.left = midX + 'px';
widget.querySelector('#label-arousal-bottom').style.top = (contentMaxY + labelGap2) + 'px';
widget.querySelector('#label-valenz-left').style.left = (contentMinX - labelGap2) + 'px';
widget.querySelector('#label-valenz-left').style.top = midY + 'px';
widget.querySelector('#label-valenz-right').style.left = (contentMaxX + labelGap2) + 'px';
widget.querySelector('#label-valenz-right').style.top = midY + 'px';

// ---- pan & zoom ----
const FRAME_PAD = 50;
const frameX0 = contentMinX - labelGap2 - FRAME_PAD, frameX1 = contentMaxX + labelGap2 + FRAME_PAD;
const frameY0 = contentMinY - labelGap2 - FRAME_PAD, frameY1 = contentMaxY + labelGap2 + FRAME_PAD;

let scale = 0.5, tx = 0, ty = 0, minScale = 0.3;

function applyTransform(){
  world.style.transform = `translate(${tx}px, ${ty}px) scale(${scale})`;
}

// Keeps the framed content from being scrolled/zoomed further out than the
// starting overview, and re-centers it whenever it would fit fully on an axis.
function clampPan(){
  const vw = viewport.clientWidth, vh = viewport.clientHeight;
  const frameW = (frameX1-frameX0)*scale, frameH = (frameY1-frameY0)*scale;
  if (frameW <= vw + 0.5){
    tx = (vw - frameW)/2 - frameX0*scale;
  } else {
    const minTx = vw - frameX1*scale, maxTx = -frameX0*scale;
    tx = Math.min(maxTx, Math.max(minTx, tx));
  }
  if (frameH <= vh + 0.5){
    ty = (vh - frameH)/2 - frameY0*scale;
  } else {
    const minTy = vh - frameY1*scale, maxTy = -frameY0*scale;
...
function fitInitial(){
  const vw = viewport.clientWidth, vh = viewport.clientHeight;
  scale = Math.min(vw/(frameX1-frameX0), vh/(frameY1-frameY0));
  minScale = scale;
  tx = vw/2 - ((frameX0+frameX1)/2)*scale;
  ty = vh/2 - ((frameY0+frameY1)/2)*scale;
  applyTransform();
}
...
function zoomAt(clientX, clientY, factor){
  ...
  scale = Math.min(3, Math.max(minScale, scale*factor));
  tx = px - wx*scale;
  ty = py - wy*scale;
  clampPan();
  applyTransform();
...
viewport.addEventListener('touchmove', e => {
  ...
  if (touchState.mode === 'pan' && e.touches.length === 1){
    ...
    scale = Math.min(3, Math.max(minScale, touchState.scale*factor));
    clampPan();
    applyTransform();
  }
}, {passive:true});
...
function focusOn(ids){
  ...
  scale = Math.min(3, Math.max(minScale, fitScale));
